...
function initTmap(sList){
   ...
   // 현재 위치정보 딱 한번 얻기
	...
	for (var i = 0; i< sList.length; i++){//for문을 통하여 배열 안에 있는 값을 마커 생성
		if(i == 0){
			 img = 'resources/img/number/one.png';
		}
		if(i == 1){
			img = 'resources/img/number/two.png';
		}
		if(i == 2){
			img = 'resources/img/number/three.png';
		}
		if(i == 3){
			 img = 'resources/img/number/four.png';
		}
		if(i == 4){
			 img = 'resources/img/number/five.png';
		}
		if(i == 5){
			img = 'resources/img/number/six.png';
		}
		if(i == 6){
			img = 'resources/img/number/seven.png';
		}
		if(i == 7){
		 	img = 'resources/img/number/eight.png';
		}
		if(i == 8){
			 img = 'resources/img/number/nine.png';
		}
		if(i == 9){
			 img = 'resources/img/number/ten.png';
		}
	...
	function onOverMarker(evt) {
		...
		icon = new Tmap.Icon(img,size, offset); // 마커 아이콘 지정
	...
	function onClickMarker(evt) {
		...
		var seslonlat = new Tmap.LonLat(this.marker).transform("EPSG:3857", "EPSG:4326");//클릭한 부분의 ViewPorPx를 LonLat로 변환합니다
		console.log(seslonlat); */
