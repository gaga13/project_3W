...
function reset(event) {
    ...
    cha.classList.remove('playing');

    clearTimeout(timer);
    timer = setTimeout( () => {
    	cha.classList.remove('look-away', 'down', 'up');
        rotate3d(0,0,0,0);
    }, 1 );
}
...
function look(event) {
    ...
    const chaRect = cha.getBoundingClientRect();
    ...
    const distCaret = chaRect.left + chaRect.width/2 - inputRect.left - caretPos;
    const distInput = chaRect.top + chaRect.height/2 - inputRect.top;
...
function lookAway(event) {
    ...
    cha.classList.add( 'look-away', distInput < 0 ? 'up' : 'down' );

    clearTimeout(timer);
    timer = setTimeout( () => {
        cha.classList.add( 'playing' );
    }, 300);
}
